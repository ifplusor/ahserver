...
def server(
    ...
    kwargs = {"app": app, "host": host, "port": port, "loop": loop, "max_workers": thread_nums}
...
def serve(app, host, port, sock, ssl_context=None, loop=None, max_workers=None):
    import asyncio
    ...
    add_dispatcher(WSGIDispatcher(loop, app, host, port, on_https=ssl_context is not None, max_workers=max_workers))
    ...
    # 创建 TCP Server
    if sock is None:
        server_coro = loop.create_server(protocol_factory=http_protocol_factory, host=host, port=port)
    else:
        server_coro = loop.create_server(protocol_factory=http_protocol_factory, sock=sock)
